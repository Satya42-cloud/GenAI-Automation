...
if region:
    ...
    route_id = st.selectbox("Select Route ID", route_options, key="route_id")

truck_type = st.selectbox("Select Truck Type", TRUCK_TYPES, key="truck_type")
...
if st.session_state.get("route_id") and st.session_state.get("truck_type"):
    ...
    combos = get_truck_combos(st.session_state["route_id"], st.session_state["truck_type"])
    combo_data = []

    for combo in combos:
        count = st.number_input(f"{combo} - Truck Count", min_value=0, key=f"{combo}_count")
        price = st.number_input(f"{combo} - Price", min_value=0.0, key=f"{combo}_price")
        combo_data.append({"route_id": st.session_state["route_id"],
                           "truck_type": st.session_state["truck_type"],
                           "combo": combo, "truck_count": count, "price": price})
